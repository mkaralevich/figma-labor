	...
	switch (command) {
		...
		case "set_layout": {
			...
			if (!("layoutMode" in node))
				throw new Error("Node does not support auto-layout (must be a frame or component)");
			const p = params;
		...
		case "run_script": {
			...
			// on ANY async API call (loadFontAsync, getNodeByIdAsync, etc.).
			//
...
async function safeGetNodeById(id) {
	...
	// when passed to getNodeByIdAsync — use findOne as a fallback.
	if (id.includes(";")) {
